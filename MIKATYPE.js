/* ***************************************************************************** */
/* MIKA's touch typing trainer  Javascript source code                           */
/*                                                    Ver2.06.01 2024/11/15      */
...
MIKA_mes0="●●●  Mika's Touch Typing Trainer Ver2.06.01  ●●●";
MIKA_mes0a="●●●  Position Practice  ●●●";
MIKA_mes0b="●●●  Random Practice  ●●●";
MIKA_mesta="●●●  %s  ●●●";
MIKA_mes0c="●●●  English Word Practice  ●●●";
MIKA_mes0d="●●●  Japanese Romaji Practice  ●●●";
MIKA_mestb="●● Position Practice (%s) ●●";
MIKA_mestc="●● Random Practice (%s) ●●";
MIKA_mesi1="Press Enter key to try again";
MIKA_mesi2="Press ESC key to return to Menu";
MIKA_mesi3="Congratulations You have updated your speed record";
MIKA_abort_mes="Press ESC key to cancel";
MIKA_return_mes="Press ESC key to return to Menu";
MIKA_key_type_mes="．．";
MIKA_keymes1="Press Space key to hide Keyguide";
MIKA_keymes2="Press Space key to show Keyguide";
MIKA_keymes3="Please press Space key to hide Keyguide and try again";
MIKA_keymes4="Please press Space key to show Keyguide and try again";
MIKA_mest2="Practice item   Typing speed per minute         Date        Practice time";
MIKA_menu_mes_s=[ /* 初期メニュー メニュー項目 */
		"Position Practice",
		"Random Practice",
		"English Word Practice",
		"Japanese Romaji Practice",
		"Show Practice Record",
		"Clear Practice Record",
	];
...
//		type_mes=String.format("ミスタッチ%3d回",MIKA_type_err_count); /* エラーカウントメッセージ作成 */
		var type_mes;
		type_mes='Error count'+formatd(MIKA_type_err_count,3); /* エラーカウントメッセージ作成 */
		if(flag==0)
		{
...
// flag=0 表示 flag=1 消去  flag=2 メッセージと共に数値を消去
	{
		disperrorcount(g,flag,5,53); /* 表示位置を指定してエラー回数表示 */
	}
...
// flag=0 表示 flag=1 数値のみ消去 flag=2 メッセージと共に数値を消去
{
		...
		type_mes='Type count '+formatd(MIKA_type_count,3); /* 正解数メッセージ作成 */
		if(flag==0)
		{
...
// flag=0 表示 flag=1 消去 
	{
		...
		type_mes="It took"+formatd(MIKA_type_speed_time,4)+"sec to finish"; /* 表示メッセージ作成 */
...
// flag=0 表示 flag=1 消去 
	{
		...
		cslput(g,20*16,10*8,MIKA_keymes3); /*「この次は、スペースキーを押してキーガイドの表示を消して練習してみましょうね」 のメッセージを表示 */
		else /* キーガイドメッセージ表示フラグが 2の場合 */
		cslput(g,20*16,10*8,MIKA_keymes4); /*「この次は、スペースキーを押してキーガイドを表示して練習してみましょうね」 のメッセージを表示 */
	}
...
function dispmaxspeedrecord(g,c,i1,j1,i2,j2) /* ランダム練習 英単語練習 ローマ字練習の 最高入力速度と 達成日を表示 */
	{
			var a,b;
			cslcolor(g,MIKA_green); /* 表示色を緑に設定 */
			a=c+formatf1(MIKA_type_speed_record[MIKA_type_kind_no],6)+"/min"; /* 最高速度メッセージ作成 */
			cslput(g,i1*16,j1*8,a); /* 最高速度メッセージ表示 */
			b="Date "+MIKA_type_date_record[MIKA_type_kind_no]; /* 達成日メッセージ作成 */
...
function disptrain(g,mest) /* ランダム練習 英単語練習 実行画面の表示 */
{
		...
		cslcolor(g,MIKA_green); /* 表示色を緑に設定 */
		cslput(g,4*16,1*8,"Time Limit 60sec"); /* 制限時間を表示 */
		if (MIKA_p_count[MIKA_type_kind_no]!=0) /* 練習回数がゼロでない場合 */
		{
...
function dispseiseki(g) /* 成績表示 */
	{
		...
		aa="Past Practice Time  "+a; /* 前回までの合計練習時間のメッセージ作成 */
		...
		aa="Current Elapsed Time"+a; /* 今回練習時間のメッセージを作成 */
		cslcolor(g,MIKA_green); /* 表示色を緑色に設定 */
...
function menexe(g,menu_mes,menu_cord,menu_function,sel_flag,menut)
	{
		...
		var x;
		var y;
		mesi5="Press Number key to select Menu";
...
//			if(MIKA_exec_func_no==9999) procexit(); /* 機能番号が 9999の時は終了 */
			...
			{
					if(window.confirm("Are you sure to clear Practice Record?")) /* 成績を消去してもいいですかのダイアログを表示 */
					{
						seisekiclear(); /* 成績をクリア */
					}
...
function dispretrymessage(g,flag) /* リトライメッセージ表示 flag=0 表示を行う flag=1 表示を消去 */
{
	...
	cslput(g,22*16,24*8,MIKA_mesi1); /* 「もう一度練習するときはリターンキーまたは、Enterキーを押してください」のメッセージを表示 */
	cslput(g,23*16,24*8,MIKA_mesi2); /* 「メニューに戻るときはESCキーを押してください」のメッセージを表示 */
}
...
// flag=0 表示 flag=1 消去
	{
		...
		a="Typing Speed Alphabet"+formatf1(MIKA_type_speed2,6); /* 打鍵数を文字列に変換 */
		offset_a=0;
		b="Romaji"+formatf1(MIKA_type_speed,6)+"/min"; /* 入力速度を文字列に変換 */
		offset_b=0;
		cslput(g,5*16,(20+offset_a)*8,a); /* 打鍵数を表示 */
		cslput(g,5*16,(48+offset_b)*8,b); /* 入力速度を表示 */
	}
...
// u_flag=0 練習経過時間を2桁の整数で表示 flag=1 練習経過時間を小数点以下二桁まで表示
	{
		...
		{
				a="Elapsed Time"+formatd(type_speed_time,2)+"sec"; /* 練習経過時間を整数で表示 */
		}
		else /* 打ち切りフラグが1の場合 */
		{
				a="Elapsed Time"+formatf2(type_speed_time,5)+"sec"; /* 練習経過時間を小数点以下二桁まで表示 */
		}
		return a;
	}
...
// flag=0 表示 flag=1 消去 
	{
		...
		cslput(g,5*16,1,a); /* 文字列の表示あるいは消去 */
	}
...
function dispupmes(g) /* タイプ速度を更新したときのメッセージを表示 */
	{
		cslcolor(g,MIKA_green); /* 表示色を緑色に設定 */
		cslput(g,20*16,16*8,MIKA_mesi3); /* 指定位置に「おめでとう、記録を更新しました」のメッセージを表示 */
	}
